...
from my_control_room import COLOUR_WHITE, COLOUR_BLACK, COLOUR_PINK, COLOUR_PURPLE, COLOUR_CYAN, COLOUR_YELLOW, COLOUR_DARKRED, COLOUR_SKYPINK, FIXED_COLOUR, INCORRECT_COLOUR, WIDTH, HEIGHT, LOCATION, SIZE, L_SIZE 

from my_tester import TO_CHECK_WITH

from my_tabs import Tab
...
# This is the state of the playing function.
      ...
      for b in range(3):
        ...
        for c in range(3):
          for d in range(3):
            index_a = a * 3 + c
            index_b = b * 3 + d
            if self.grid[index_b][index_a] in varieties:
              varieties.remove(self.grid[index_b][index_a])
            else:
              if [index_a, index_b] not in self.fixed_cells and [index_a, index_b] not in self.invalid_cells:
                self.invalid_cells.append([index_a, index_b])
              if [index_a, index_b] in self.fixed_cells:
                for m in range(3):
                  for n in range(3):
                    index_aa = a * 3 + m
                    index_bb = b * 3 + n
                    if self.grid[index_bb][index_aa] == self.grid[index_b][index_a] and [index_aa, index_bb] not in self.fixed_cells:
                      self.invalid_cells.append([index_aa, index_bb])
    ...
    for index_b, row in enumerate(self.grid):
      ...
      for index_a in range(9):
        if self.grid[index_b][index_a] in varieties:
          ...
          if [index_a, index_b] in self.fixed_cells:
            for t in range(9):
              if self.grid[index_b][t] == self.grid[index_b][index_a] and [t, index_b] not in self.fixed_cells:
                self.invalid_cells.append([t, index_b])
        ...
        if self.grid[index_b][index_a] in varieties:
          ...
          if [index_a, index_b] in self.fixed_cells:
            for t, row in enumerate(self.grid):
              if self.grid[t][index_a] == self.grid[index_b][index_a] and [index_a, t] not in self.fixed_cells:
                self.invalid_cells.append([index_a, t])

  # This function imports the sudoku board (with fixed numbers) from the given website
  def puzzle_game(self, difficulty_level):
    html_doc = requests.get("https://nine.websudoku.com/?level={}".format(difficulty_level)).content
    soup = BeautifulSoup(html_doc, "html.parser")
    ids = ['f00', 'f01', 'f02', 'f03', 'f04', 'f05', 'f06', 'f07', 'f08', 'f10', 'f11', 'f12', 'f13', 'f14', 'f15', 'f16', 'f17', 'f18', 'f20', 'f21', 'f22', 'f23', 'f24', 'f25', 'f26', 'f27', 'f28', 'f30', 'f31', 'f32', 'f33', 'f34', 'f35', 'f36', 'f37', 'f38', 'f40', 'f41', 'f42', 'f43', 'f44', 'f45', 'f46', 'f47',
    'f48', 'f50', 'f51', 'f52', 'f53', 'f54', 'f55', 'f56', 'f57','f58', 'f60', 'f61', 'f62', 'f63', 'f64', 'f65', 'f66', 'f67', 'f68', 'f70', 'f71', 'f72', 'f73', 'f74', 'f75', 'f76', 'f77', 'f78', 'f80', 'f81', 'f82', 'f83', 'f84', 'f85', 'f86', 'f87', 'f88']
  ...
  def rainfall_effect(self):
    mixer.music.load("rainfall.wav")
    mixer.music.play(-1)

  def bird_chirps_effect(self):
    mixer.music.load("bird_chirps.wav")
    mixer.music.play(-1)

  def river_flow_effect(self):
    mixer.music.load("river_flow.wav")
    mixer.music.play(-1)
  
  def evil_sound_effect(self):
    mixer.music.load("evil_sound.wav")
    mixer.music.play(-1)
  ...
  def colour_invalid_boxes(self, window, invalid):
    for box in invalid:
      pg.draw.rect(window, INCORRECT_COLOUR, (box[0] * SIZE+ LOCATION[0], box[1] * SIZE + LOCATION[1], SIZE, SIZE))

  # This function fills the colour in the fixed boxes.
  def colour_fixed_boxes(self, window, fixed):
    for cell in fixed:
      pg.draw.rect(window, FIXED_COLOUR, (cell[0] * SIZE + LOCATION[0], cell[1] * SIZE + LOCATION[1], SIZE, SIZE))
  ...
  def import_number(self, window):
    for index_b, row in enumerate(self.grid):
      for index_a, num in enumerate(row):
        if num != 0:
          pos = [(index_a * SIZE) + LOCATION[0], (index_b * SIZE) + LOCATION[1]]
          self.from_text_to_screen(window, str(num), pos)
  #
  def make_selection(self, window, position):
    pg.draw.rect(window, COLOUR_PINK, ((position[0] * SIZE) + LOCATION[0], (position[1] * SIZE) + LOCATION[1], SIZE, SIZE))
  ...
  def make_grid(self, window):
    pg.draw.rect(window, COLOUR_BLACK, (LOCATION[0], LOCATION[1], WIDTH - 150, HEIGHT - 150), 2)
    for x in range(9):
      pg.draw.line(window, COLOUR_BLACK, (LOCATION[0] + (x * SIZE), LOCATION[1]), (LOCATION[0] + (x * SIZE), LOCATION[1] + 450), 2 if x % 3 == 0 else 1)
      pg.draw.line(window, COLOUR_BLACK, (LOCATION[0], LOCATION[1]+(x * SIZE)), (LOCATION[0] + 450, LOCATION[1] ++ (x * SIZE)), 2 if x % 3 == 0 else 1)

  # This function checks if the mouse pointer in the small boxes or not.
  def mouse_on_box(self):
    if self.mouse_position[0] < LOCATION[0] or self.mouse_position[1] < LOCATION[1]:
      return False
    if self.mouse_position[0] > LOCATION[0] + L_SIZE or self.mouse_position[1] > LOCATION[1] + L_SIZE:
      return False
    return ((self.mouse_position[0] - LOCATION[0]) // 50, (self.mouse_position[1] - LOCATION[1]) // 50)

  # These function helps to customizes the colours, size and the text written on the difficulty tabs.
  def button_loader(self):
    self.button_player.append(Tab(40, 40, 200, 30, function = self.cells_checker , colour = COLOUR_PURPLE, text = "Check My Game"))
    self.button_player.append(Tab(250, 40, 120, 30, colour = COLOUR_CYAN, function=self.puzzle_game, params="1", text="EASY"))
    self.button_player.append(Tab(380, 40, 100, 30, colour = COLOUR_YELLOW, function=self.puzzle_game, params="2", text="MEDIUM"))
    self.button_player.append(Tab(490, 40, 100, 30, colour = COLOUR_DARKRED, function=self.puzzle_game, params="4", text="EVIL"))
    # self.button_player.append(Tab(40, 80, 160, 30, colour = COLOUR_SKYPINK, text="Choose a music :: "))
    self.button_player.append(Tab(205, 80, 70, 30, colour = COLOUR_SKYPINK, function = self.rainfall_effect, text="Rainfall"))
    self.button_player.append(Tab(280, 80, 100, 30, colour = COLOUR_SKYPINK, function = self.bird_chirps_effect, text="Bird Chirps"))
    self.button_player.append(Tab(385, 80, 100, 30, colour = COLOUR_SKYPINK, function = self.river_flow_effect, text="Water Flow"))
    self.button_player.append(Tab(490, 80, 100, 30, colour = COLOUR_SKYPINK, function = self.evil_sound_effect, text="Evil Sound"))
